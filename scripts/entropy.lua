...
function verify_entropy(relname, tablename, tableam, is_encrypted, filename)
	...
	info("TABLE: " .. tablename .. " REL: " .. relname .. " FILE: " .. filename .. " AM: " .. tableam .. " IS_ENCRYPTED: " .. is_encrypted .. " ENTROPY: " .. entropy);
	if tableam == "tde_heap" then
		...
		if entropy > 0.8 then
			warning("FILE " .. filename .. " FOR TABLE " .. tablename .. " shouldn't be encrypted, but has high entropy!");
		end
	end
...
function db_files_entropy(w, datadir)
	list_tables = "select pg_relation_filepath(c.oid), c.relname, a.amname, c.oid, pg_tde_is_encrypted(c.oid) from pg_class c join pg_am a ON a.oid = c.relam where relkind in ('r', 'm') AND c.relname not like 'pg_%' AND c.relname not like 'sql_%';";
	...
	for _, rec in ipairs(allData) do
		...
		list_deps = "select pg_relation_filepath(c.oid), c.relname, pg_tde_is_encrypted(c.oid) from pg_class c join pg_attribute a on a.attrelid = " .. oid .. " join pg_depend d ON d.objid = c.oid and d.deptype in ('i', 'a') and a.attrelid = " .. oid .. " where d.refobjid=" .. oid .. " group by c.oid";
