function init_pg_tde_only_for_db(sqlconn)
	...
	sqlconn:execute_query("SELECT pg_tde_create_key_using_database_key_provider('principal-key', 'reg_file');")
	sqlconn:execute_query("SELECT pg_tde_set_key_using_database_key_provider('principal-key', 'reg_file');")
end
