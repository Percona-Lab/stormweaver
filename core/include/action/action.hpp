...
public:
  ActionException(std::string errorName, std::string message)
      : errorName(std::move(errorName)), message(std::move(message)) {}

  [[nodiscard]] const char *what() const noexcept override {
    return message.c_str();
  }

  [[nodiscard]] const std::string &getErrorName() const noexcept {
    return errorName;
  }
