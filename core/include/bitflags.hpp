...
// From: https://voithos.io/articles/type-safe-enum-class-bit-flags/
// With some modifications
// TODO: maybe we could have a better solution, but this is a good quick
// starting point
...
public:
  ...
  [[nodiscard]] constexpr bool IsSet(T v) const {
  ...
  [[nodiscard]] constexpr std::vector<T> All() const {
    std::vector<T> result;
    for (auto const &v : rfl::get_underlying_enumerator_array<T>()) {
      if (IsSet(T(v.second))) {
        result.push_back(T(v.second));
      }
    }
    return result;
  }

  static constexpr BitFlags<T> AllSet() {
    BitFlags<T> result;
    for (auto const &v : rfl::get_underlying_enumerator_array<T>()) {
      result.Set(T(v.second));
    }
    return result;
  }
  ...
  [[nodiscard]] constexpr UnderlyingT ToRaw() const { return flags_; }
