...
bool Table::operator==(const Table &other) const {
  ...
  for (const auto &column : columns) {
    if (std::ranges::find(other.columns, column) == other.columns.end()) {
      return false;
    }
  }

  return std::ranges::all_of(indexes, [&](const auto &index) {
    return std::ranges::find(other.indexes, index) != other.indexes.end();
  });
}
...
bool Table::hasReferenceTo(std::string const &tableName) const {
  return std::ranges::any_of(columns, [&](const auto &column) {
    return column.foreign_key_references == tableName;
  });
}
...
void Table::updateReferencesTo(std::string const &oldTableName,
  ...
  for (auto &column : columns) {
    if (column.foreign_key_references == oldTableName) {
      column.foreign_key_references = newTableName;
    }
  }
}
...
Metadata::Reservation::Reservation(Metadata *storage, Metadata::table_t table,
    ...
    : storage_(storage), table_(std::move(table)), drop_(drop), index_(index),
      lock_(std::move(lock)) {}
...
Metadata::Reservation Metadata::createTable() {
  ...
  if (data_.reservedSize < limits::maximum_table_count) {
    ...
    return {this, std::make_shared<Table>(), false, npos, {}};
  }

  return {nullptr, nullptr, false, npos, {}};
}
...
Metadata::Reservation Metadata::alterTable(index_t idx) {
  ...
  return {this, std::make_shared<Table>(*table), false, idx, std::move(mtx)};
}
...
Metadata::Reservation Metadata::dropTable(index_t idx) {
  ...
  return {this, table, true, idx, std::move(mtx)};
}
...
bool Metadata::operator==(const Metadata &other) const {
  ...
  std::ranges::sort(this_tables, table_comparator);
  std::ranges::sort(other_tables, table_comparator);
...
std::string Metadata::debug_dump() const {
  ...
  std::ranges::sort(tables, table_comparator);
